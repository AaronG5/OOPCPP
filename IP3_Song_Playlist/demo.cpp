...
int main() {
   try {
      ...
      jazz->addSong("Tenderly", "Chet Baker", 6, 38);
      jazz->addSong("Look To The Sky", "Antonio Carlos Jobim", 2, 20);
      jazz->addSong("Prelude In E Minor", "Gerry Mulligan Sextet", 4, 17);
      jazz->addSong("Fiasco", "Gus Viseur", 2, 58);
      ...
      brazilianJazz->addSong("Flor Do Brazil", "Jean Bonal", 2, 12);
      ...
      jazz->addSong("When Sunny Gets Blue", "McCoy Tyner", 4, 42);
      jazz->addSong("It Never Entered My Mind", "Miles Davis", 4, 3);
      jazz->setSortMethod(SORT_BY_LENGTH); // POLYMORPHISM
      ...
      jazz->setSortMethod(SORT_BY_TITLE); // POLYMORPHISM
      jazz->sortPlaylist(); // POLYMORPHISM
      jazz->getPlaylistInfo();
